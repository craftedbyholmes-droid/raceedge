...
function PNL({ val }) {
  ...
  return <span style={{ color: v >= 0 ? "var(--green)" : "var(--red)", fontWeight: 700 }}>
...
export default function TipstersPage() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => fetch("/api/stats").then(r => r.json()).then(setData), 600000);
  ...
  const personas = {
    AJ: { name: "Rambling Robbie", cls: "robbie", colour: "var(--robbie)", spec: "National Hunt — Chase and Hurdle",
          bio: "Jumps specialist with 30 years trackside. Knows every trainer's prep routine and stable confidence levels." },
    TC: { name: "Punter Pat",      cls: "pat",    colour: "var(--pat)",    spec: "Flat Racing Specialist",
          bio: "Grew up near Newmarket. Stats-driven flat expert with a sharp eye for horses improving through the weights and market moves." },
  };
  ...
  const pnl = id => (season[id]?.total_returned || 0) - (season[id]?.total_staked || 0);
  ...
  return (
    <div style={{ paddingTop: 16 }}>
          ...
          {["AJ","TC"].map(id => {
            ...
            return (
              <div key={id} style={{ flex: 1, minWidth: 140, background: "var(--surface2)", borderRadius: "var(--radius)", padding: 14, borderTop: "3px solid " + p.colour }}>
                <div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 17, marginBottom: 10 }}>{p.name}</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  ...
                  <div><div style={{ fontSize: 10, color: "var(--muted)" }}>P&L</div><PNL val={pnl(id)} /></div>
                </div>
              </div>
            );
          })}
      ...
      {/* Today picks */}
      <h2 className="section-title" style={{ fontSize: 19 }}>Today's Tips</h2>
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        {["AJ","TC"].map(id => {
          const p = personas[id];
          const pick = (today[id] || [])[0];
          return (
            <div key={id} className={"tipster-card " + p.cls} style={{ flex: 1, minWidth: 260 }}>
                ...
                {!pick ? (
                  ...
                  <div className="pick-card" style={{ margin: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 20, marginTop: 10, flexWrap: "wrap" }}>
                      <div><div style={{ fontSize: 10, color: "var(--muted)" }}>ODDS</div><div className="pick-odds">{pick.odds || "N/A"}</div></div>
                      <div><div style={{ fontSize: 10, color: "var(--muted)" }}>SCORE</div><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 24, color: "var(--gold)" }}>{Math.round(pick.score || 0)}</div></div>
                    </div>
                    {pick.tip_text && (
                      <div className="pick-tip">"{pick.tip_text}"</div>
                    )}
      ...
      {plan === "free" ? (
        ...
        <>
          <h2 className="section-title" style={{ fontSize: 19, marginTop: 8 }}>Pick Archive</h2>
          <div className="tabs">
          ...
          {byMonth(tab).map(([month, mPicks]) => {
            ...
            return (
              <div key={month} style={{ marginBottom: 8 }}>
                <div className="month-header" onClick={() => setMonths(m => Object.assign({}, m, { [month]: !m[month] }))}>
                  <span>{fmtMonth(month)} <span style={{ fontSize: 12, color: "var(--muted)", fontWeight: 400 }}>({mPicks.length} picks)</span></span>
                ...
                {open && mPicks.map((p, i) => {
                  const pv = pickPnL(p);
                  return (
                    <div key={i} style={{ background: "var(--surface2)", borderRadius: "var(--radius)", padding: "10px 14px", marginBottom: 4, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                      <div style={{ flex: 1, minWidth: 160 }}>
                        <div style={{ fontFamily: "Barlow Condensed, sans-serif", fontWeight: 700, fontSize: 15 }}>{p.horse_name}</div>
                        <div style={{ fontSize: 11, color: "var(--muted)" }}>{p.course} · {p.race_date}</div>
                      </div>
                      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        <span style={{ fontSize: 14, color: "var(--gold)", fontWeight: 700 }}>{p.odds || "N/A"}</span>
