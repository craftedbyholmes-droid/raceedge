...
async function genTip(runner, persona, race) {
  try {
    const form    = runner.form     || 'unknown';
    const trainer = runner.trainer  || 'unknown trainer';
    const jockey  = runner.jockey   || 'unknown jockey';
    const odds    = runner.best_fractional || 'N/A';
    const going   = race?.going     || 'unknown going';
    const dist    = race?.distance_furlongs ? race.distance_furlongs + 'f' : 'unknown distance';

    const prompt = 'You are ' + persona.name + ', an experienced UK horse racing tipster. ' +
      'Write 2-3 sentences explaining WHY ' + runner.horse_name + ' is your top pick at ' + runner.course + ' today. ' +
      'The horse is ridden by ' + jockey + ', trained by ' + trainer + ', has form figures of ' + form + ', ' +
      'is available at ' + odds + ' and runs over ' + dist + ' on ' + going + ' ground. ' +
      'Be specific and confident. Mention the jockey, trainer form or going suitability where relevant. ' +
      'Sound like a real expert, not a robot.';
...
export async function GET() {
  ...
  for (const runner of (runners || [])) {
    ...
    await supabase.from('scores').upsert({
      runner_id: runner.runner_id, race_id: runner.race_id, race_date: runner.race_date, scores, total,
    }, { onConflict: 'runner_id' });
  }
  log.push('scored ' + (runners || []).length);
  ...
  for (const [key, date] of [['races_today', today], ['races_tomorrow', tomorrow]]) {
    const { data: races } = await supabase.from('races').select('*').eq('race_date', date).order('off_time');
    const built = [];
    for (const race of (races || [])) {
      if (date === today && race.off_time && race.off_time < nowTime) continue;
      const { data: rr } = await supabase.from('runners').select('*, scores(total)').eq('race_id', race.race_id).limit(30);
      const sorted = (rr || []).map(r => {
        const o = bestOdds(r.odds_raw?.odds || []);
        return Object.assign({}, r, { total: r.scores?.total || 0, best_fractional: o.fractional, best_decimal: o.decimal, best_bookmaker: o.bookmaker });
  ...
  // STEP 4: Picks with detailed tips
  const { data: allRunners } = await supabase.from('runners').select('*, scores(total)').eq('race_date', today);
  const { data: allRaces }   = await supabase.from('races').select('*').eq('race_date', today);
  const raceMap = {};
  for (const r of (allRaces || [])) raceMap[r.race_id] = r;
  const enriched = (allRunners || []).map(r => Object.assign({}, r, { total: r.scores?.total || 0 }));

  const topPicks = {};
  for (const id of Object.keys(PERSONAS)) {
    const persona = PERSONAS[id];
    for (const pick of selectPicks(enriched, id)) {
      const race      = raceMap[pick.race_id] || {};
      const pickOdds  = bestOdds(pick.odds_raw?.odds || []);
      const pickWithOdds = Object.assign({}, pick, { best_fractional: pickOdds.fractional });
      const tipText   = await genTip(pickWithOdds, persona, race);
      await supabase.from('persona_picks').upsert({
        ...
        course: pick.course || race.course, race_type: pick.race_type,
        race_date: today, score: pick.total, score_gap: pick.score_gap,
        is_best_pick: true, tip_text: tipText, odds: pickOdds.fractional || 'N/A',
      }, { onConflict: 'pick_id' });
      topPicks[id] = { horse_name: pick.horse_name, course: pick.course || race.course, race_type: pick.race_type, score: pick.total, odds: pickOdds.fractional || 'N/A', tip_text: tipText };
    }
  }
  const allTop = Object.values(topPicks);
  if (allTop.length) {
    const free = allTop.sort((a, b) => b.score - a.score)[0];
    await supabase.from('cache').upsert({ key: 'free_tip_today', value: free, date: today, updated_at: new Date().toISOString() }, { onConflict: 'key' });
  }
  log.push('picks: ' + Object.keys(topPicks).length);

  // STEP 5: Yesterday results
  try {
