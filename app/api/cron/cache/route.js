...
export async function GET() {
  ...
  const nowTime = String(nowBST.getHours()).padStart(2, '0') + ':' + String(nowBST.getMinutes()).padStart(2, '0');
  ...
  for (const [key, date] of [['races_today', today], ['races_tomorrow', tomorrow]]) {
    ...
    for (const race of (races || [])) {
      // Skip races that have already gone off today
      ...
      const sorted = (rr || [])
        .map(r => {
          // odds_raw is the full runner object from API - odds array is at odds_raw.odds
