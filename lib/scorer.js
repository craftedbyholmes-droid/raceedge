...
function marketSignal(oddsArray) {
  if (!oddsArray || !oddsArray.length) return 50;
  const known = ['Bet365','William Hill','Ladbrokes','Coral','Paddy Power','Betfred'];
  let best = null;
  for (const bm of known) {
    const found = oddsArray.find(o => o.bookmaker === bm && o.decimal && o.decimal !== '-');
...
function officialRating(runner) {
  ...
  if (!ofr || ofr <= 0) return parseInt(runner.rpr) ? Math.min(100, parseInt(runner.rpr) / 1.3) : 50;
