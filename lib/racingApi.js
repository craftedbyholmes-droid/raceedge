...
export async function fetchRacecards(day = 'today') {
  ...
  if (!res.ok) {
    throw new Error('RacingAPI racecard ' + res.status + ': ' + await res.text());
  }
  const data = await res.json();
  const filtered = (data.racecards || []).filter(isUKIRE);
...
export async function fetchResults(date) {
  ...
  if (!res.ok) {
    throw new Error('RacingAPI results ' + res.status + ': ' + await res.text());
  }
  const data = await res.json();
  const filtered = (data.results || data.races || []).filter(isUKIRE);
